...
import { loadVenues, getVenueStats } from "@/lib/venueStore";
 ...
 * Recommendations are keyed off venues matched to events by location name.
 */
export async function generateInsights(): Promise<DashboardInsight[]> {
  const insights: DashboardInsight[] = [];
  const venues = loadVenues();
  const allEvents = loadEvents();
  const venueStats = venues.map(v => ({ venue: v, stats: getVenueStats(v.name, allEvents) }));

  // 1. Best venue by revenue
  const booked = venueStats.filter(vs => vs.stats.completedCount > 0 && vs.stats.totalRevenue > 0);
  if (booked.length > 0) {
    const best = [...booked].sort((a, b) => b.stats.totalRevenue - a.stats.totalRevenue)[0];
    insights.push({
      type: "no_upcoming_events",
      actionableNextStep: `${best.venue.name} is your best venue (avg $${(best.stats.totalRevenue / best.stats.completedCount).toFixed(0)}/event, ${best.stats.completedCount} bookings)`,
      relatedVenueName: best.venue.name,
      priority: "high",
    });
  }

  // 2. Inactive venue — hosted before but not in 60+ days
  if (insights.length < 3) {
    const now = Date.now();
    const inactive = venueStats
      .filter(vs => vs.stats.completedCount > 0 && vs.stats.lastEventDate)
      .map(vs => ({ vs, daysAgo: Math.floor((now - new Date(vs.stats.lastEventDate!).getTime()) / (1000 * 60 * 60 * 24)) }))
      .filter(x => x.daysAgo >= 60)
      .sort((a, b) => b.daysAgo - a.daysAgo)[0];
    if (inactive) {
      insights.push({
        type: "low_revenue_trend",
        actionableNextStep: `${inactive.vs.venue.name} hasn't hosted in ${inactive.daysAgo}d. Time for a check-in?`,
        relatedVenueName: inactive.vs.venue.name,
        priority: "medium",
      });
    }
  }

  // 3. Instagram follows with no bookings yet
  if (insights.length < 3) {
    const igLead = venueStats.find(vs => vs.venue.instagramFollowing && vs.stats.totalCount === 0);
    if (igLead) {
      insights.push({
        type: "inventory_low",
        actionableNextStep: `${igLead.venue.name} follows on Instagram. Reach out about hosting?`,
        relatedVenueName: igLead.venue.name,
        priority: "medium",
      });
    }
  }

  // 4. A venue with no events yet
  if (insights.length < 3) {
    const unbooked = venueStats.find(vs => vs.stats.totalCount === 0);
    if (unbooked) {
      insights.push({
        type: "inventory_low",
        actionableNextStep: `${unbooked.venue.name} isn't booked yet. Consider reaching out.`,
        relatedVenueName: unbooked.venue.name,
        priority: "low",
      });
    }
  }
  ...
  if (insights.length < 3) {
    insights.push({
      type: "inventory_low",
      actionableNextStep: `Add venues and log events to generate personalized recommendations.`,
      priority: "low",
    });
  }
